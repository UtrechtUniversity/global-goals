...
class PageFetcher:
    def __init__(self, bucket_name: str, limit_rate: int = 10) -> None:
        ...
        self._pool = Pool(pow(2, ceil(log(limit_rate) / log(2))))  # IO-Bound, thus increase pool size        self._error_detected = 0
    ...
    def fetch(self, csv_path: Path, fetch_limit: int = -1) -> None:
        ...
        with open(str(csv_path), 'r') as csv_file:
            ...
            for i, line in enumerate(reader):
                ...
                url = self._get_url(timestamp=line[0], url_key=line[1])
    ...
    def error_callback(self, ex: Exception, url_: str) -> None:
        logging.getLogger().error(f"Exception occurred while fetching {url_}")
        logging.getLogger().exception(ex)
